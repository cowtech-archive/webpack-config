...
async function resolveFile(options, key, pattern) {
    let file = utils_1.get(options, key, true);
    if (file === true) {
...
async function setupPlugins(options) {
    ...
    const analyze = utils_1.get(pluginsOptions, 'analyze', true);
    const hmr = utils_1.get(options, 'server.hot', true);
    ...
    if (manifest && utils_1.get(options.rules, 'manifest', true)) {
    ...
    if (robots && utils_1.get(options.rules, 'robots', true)) {
        plugins.push(new html_webpack_plugin_1.default({
            id: 'robots',
            filename: 'robots.txt',
            template: robots,
    ...
    if (options.environment === 'production') {
        if (utils_1.get(pluginsOptions, 'minify', true)) {
            plugins.push(new terser_webpack_plugin_1.default(utils_1.get(options, 'uglify', {})));
        }
    }
    else if (hmr) {
    ...
    if (analyze) {
        if (path_1.basename(process.argv[1]) !== 'webpack') {
            plugins.push(new webpack_bundle_analyzer_1.BundleAnalyzerPlugin({
                ...
                analyzerHost: utils_1.get(options, 'server.host', 'home.cowtech.it'),
                analyzerPort: utils_1.get(options, 'server.port', 4200) + 2,
    ...
    if (utils_1.get(swOptions, 'enabled', options.environment === 'production')) {
        ...
        if (swSrc) {
            ...
            const swDest = utils_1.get(swOptions, 'dest', 'sw.js');
            ...
            plugins.push(new ServiceWorkerEnvironment({
                ...
                debug: utils_1.get(swOptions, 'debug', options.environment !== 'production')
            }), new workbox_webpack_plugin_1.InjectManifest(Object.assign({ swSrc,
                swDest, include: exports.serviceWorkerDefaultInclude, exclude: exports.serviceWorkerDefaultExclude, importScripts: [`/${envFile}`] }, utils_1.get(swOptions, 'options', {}))));
        }
    }
