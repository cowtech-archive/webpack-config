...
async function setupServer(options) {
    ...
    if (!('https' in serverOptions)) {
        // Autodetect HTTPS
        cert = (await globby_1.default(path_1.resolve(process.cwd(), './config/ssl/(private-key|privkey).pem'))).pop();
        privkey = (await globby_1.default(path_1.resolve(process.cwd(), './config/ssl/(certificate|cert).pem'))).pop();
        https = !!(cert && privkey);
    }
    else {
